...
type WorkspaceCustomerManagedKey struct {
	...
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.keyVaultKeyId) || has(self.initProvider.keyVaultKeyId)",message="keyVaultKeyId is a required parameter"
